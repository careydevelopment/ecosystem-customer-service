node {
	def app
	def image = 'careydevelopment/ecosystem-customer-service'
	...
	try {
		stage('Clone repository') {
	    	git branch: branch,
	        	...
	        	url: 'https://github.com/careydevelopment/ecosystem-customer-service.git'
	    } 
	
		stage('Build JAR') {
	    	docker.image('maven:3.6.3-jdk-11').inside('-v /root/.m2:/root/.m2') {
	        	sh 'mvn -B -Dmaven.test.skip=true clean package'
	        	stash includes: '**/target/ecosystem-customer-service.jar', name: 'jar'
	    	}
	    }
